...
Ext.define('SMC.view.pnl_xtm_mgt_rtm', {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            items: [
                                ...
                                {
                                    xtype: 'label',
                                    html: '※ 회선점검대상 IP는 콤마 (,) 로 구분하여 <br>최대 두 개 입력 가능',
                                    width: 300
                                }
                            ]
                        },
    ...
    /*
        ...
        설 명 : RTM 사용 여부에 대한 이벤트를 설정합니다.

        수 정 :

            - (2015.01.21 김민수 : 체크박스 해제시 컴포넌트 값이 초기화되는 부분 제거)
            - (2015.09.23 김민수 : RTM 사용 체크 설정 시, 기본 값을 가져오는 코드 추가)
    */
    onCk_usertmChange: function(field, newValue, oldValue, eOpts) {
        ...
        // 정보를 불러와서 컴포넌트에 초기화 ==================================================================================================================================================
            ...
            var dat_devmain = Ext.getCmp(DEVICE_COMMON_ID.devicesetting).deviceParams;
            var dat_devethinfo = dat_devmain.network_interface.interface;

            // RTM 서버 IP 설정

            var dat_serverip = component.rtmip.getValue();
            var dat_deviceip = component.deviceip.getValue();
            var st_ethlist = component.checkethlist.getStore();

            // RTM 서버 IP가 입력이 되지 않았거나 0.0.0.0 이라면 기본정보에서 데이터를 가져와 초기화

            if(dat_serverip === '' || dat_serverip === '0.0.0.0'){

                var dat_smcip = dat_devmain.center_setup.center.first_ip;

                component.rtmip.setValue(dat_smcip);

            }

            // 인터페이스 검사 -> 장비 - 인터페이스가 설정된 것이 있다면. (None 타입이 아닌 것들을 추가. eth0 ~ eth7 까지 추가됨.)

            if(dat_deviceip === '' || dat_deviceip === '0.0.0.0'){

                for(var i = 0; i <= 7; i++){

                    // 만약, 인터페이스 갯수가 7개 이하라면 반복문 멈춤
                    ...
                    if(dat_devethinfo[i].setting['@type'] === 'Static'){

                        var dat_ipv4 = dat_devethinfo[i].ip[0]['#text'].split('/')[0];

                        component.deviceip.setValue(dat_ipv4);
            ...
            // 등록된 인터페이스가 없는 경우에만 장비의 인터페이스 정보로 초기화 함. 있으면 패스.

            if(st_ethlist.count() <= 0){
                ...
                for(var i = 0; i <= 7; i++){
                    ...
                    // 인터페이스의 타입이 None 이라면 Continue

                    if(dat_devethinfo[i].setting['@type'] === 'None'){

                        continue;

                    }

                    arr_chketh.push({	'eth' : dat_devethinfo[i].name	});

                }

                st_ethlist.loadData(arr_chketh);
        ...
        // 인터페이스 검사 ================================================================================================================================================================
            ...
            if(rtmData.setting['interface']){
                ...
                for(var i = 0; i < rtmData.setting['interface'].length; i++){
                    ...
                    arr_ethinfo.push({	'eth' : rtmData.setting['interface'][i]	});

                }

                ethCheckStore.loadData(arr_ethinfo);
        ...
        // 회선 대역폭 측정 ===============================================================================================================================================================
            ...
            if(rtmData.setting.bandwidth){
                ...
                for(var i = 0; i < rtmData.setting.bandwidth.length; i++){
                    ...
                    arr_bandinfo.push({	'eth' : rtmData.setting.bandwidth[i]	});

                }

                bandwidthCheckStore.loadData(arr_bandinfo);
    ...
    validityCheck: function() {
        ...
        var validCheckObj = {
            ...
            validateCheck : function(){
                ...
                if(component.usertm.getValue() && !component.rtmperiod.validate()){

                    Ext.Msg.show({

                        title : 'WeGuardia™ SMC 2.0',
                        msg : '정보 갱신 주기의 범위는 0 ~ 60 입니다.',
                        buttons : Ext.Msg.OK,
                        icon : Ext.Msg.ERROR
                ...
                if(component.useexceptdate.getValue()){

                    if(new Date(component.exceptstartdate.getSubmitValue()) > new Date(component.exceptenddate.getSubmitValue()))
                    {

                        Ext.Msg.show({

                            title : 'WeGuardia™ SMC 2.0',
                            msg : '시작 날짜가 종료 날짜보다 늦은 날짜로 설정 될 수 없습니다.',
                            buttons : Ext.Msg.OK,
                            icon : Ext.Msg.ERROR
                ...
                if(component.useexcepttime.getValue()){

                    if(component.exceptstartdate.getSubmitValue() === component.exceptenddate.getSubmitValue())
                    {
                        if(Number(component.exceptstarttime.getSubmitValue().split(":").join("")) >= Number(component.exceptendtime.getSubmitValue().split(":").join("")))
                        {

                            Ext.Msg.show({

                                title : 'WeGuardia™ SMC 2.0',
                                msg : '종료시간이 시작시간보다 작거나 같을 수 없습니다.',
                                buttons : Ext.Msg.OK,
                                icon : Ext.Msg.ERROR
