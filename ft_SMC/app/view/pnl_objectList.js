...
Ext.define('SMC.view.pnl_objectList', {
    ...
    onPnl_objectListAfterRender: function(component, eOpts) {
        ...
        var obj_Context_Array = [
            ...
            {
                'name' : '정책 전송',
                'itemId' : 'id_objectSendpolicy',
                'children' : [
                    {
                        'name' : '연결 장비 정책 전송',
                        'itemId' : 'id_now_objectSendpolicy',
                        'children' : null,
                        'callback' : function(){

                            var gpn_objectList = Ext.ComponentQuery.query('gridpanel[itemId=gpn_objectList]')[0];
                            var sel_objectdata = gpn_objectList.getSelectionModel().getSelection();

                            var arr_cid = [];

                            for(var i = 0, max = sel_objectdata.length; i < max; i++){

                                arr_cid.push(sel_objectdata[0].get('@cid'));

                            }

                            getSendpolicyList('referenced', arr_cid, function(res){
                    ...
                    {
                        'name' : '변경 장비 정책 전송',
                        'itemId' : 'id_change_objectSendpolicy',
                        'children' : null,
                        'callback' : function(){
        ...
        var contextCallback = function(menuInstance){
            ...
            menuInstance.down('[itemId=id_now_objectSendpolicy]').enable();
            menuInstance.down('[itemId=id_change_objectSendpolicy]').enable();
            ...
            if(tree_select)
            {
                if(!tree_select.raw._kind || tree_select.raw._kind === 'obj_ip_eth')
                {
                    ...
                    menuInstance.down('[itemId=id_now_objectSendpolicy]').disable();
                }
                else
                {
                    if(!objectList.getSelectionModel().getSelection()[0])
