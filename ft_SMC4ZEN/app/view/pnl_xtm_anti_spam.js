...
Ext.define('SMC4ZEN.view.pnl_xtm_anti_spam', {
    extend: 'Ext.panel.Panel',

    requires: [
        'SMC4ZEN.view.pnl_xtm_anti_spamViewModel',
        'SMC4ZEN.view.ctn_anti_control',
        'Ext.form.FieldSet',
        'Ext.form.RadioGroup',
    ...
    listeners: {
        afterrender: 'onPnl_xtm_anti_spamAfterRender',
        beforeclose: 'onPnl_xtm_anti_spamBeforeClose'
    },
                        ...
                        items: [
                            {
                                ...
                                valueField: 'value',
                                listeners: {
                                    afterrender: 'onCmb_ruleAfterRender'
                                }
                            },
                            {
                                xtype: 'textfield',
                            ...
                            {
                                xtype: 'ctn_anti_control',
                                margin: '0, 0, 10, 0',
                                listeners: {
                                    afterrender: 'onCtn_spam_controlAfterRender'
                                }
                            },
                            {
                                xtype: 'gridpanel',
                                ...
                                listeners: {
                                    itemclick: 'onGpn_spam_antispamlistItemClick'
                                },
    ...
    onCmb_ruleAfterRender: function(component, eOpts) {
        component.bindStore(Ext.create('Ext.data.Store', {
            ...
            'data' : [
                {	'name' : '송신자 허용', 'value' : 'Whitelist_sender'		},
                {	'name' : '수신자 허용', 'value' : 'Whitelist_recipient'		},
                {	'name' : '제목 허용', 'value' : 'Whitelist_subject'		},
                {	'name' : '송신자 차단', 'value' : 'sender'		},
                {	'name' : '수신자 차단', 'value' : 'recipient'		},
                {	'name' : '제목 차단', 'value' : 'subject'		},
                {	'name' : '내용 차단', 'value' : 'content'		},
                {	'name' : '본문 URI 차단', 'value' : 'URI'		},
                {	'name' : 'rawbody 차단', 'value' : 'rawbody'		}
            ]
        }));

        component.setValue('Whitelist_sender');
    },
    ...
    componentStorage: function() {
        ...
        obj.rblserverlist = rblspam.down('[itemId=fds_spam_rblserver]').down('[itemId=ctn_spam_rbllist]').down('[itemId=gpn_spam_rbllist]');
    ...
    initStore: function() {
        ...
        var st_rblserver = Ext.getStore('st_spam_rblserver');
        var st_rulelist = Ext.getStore('st_spam_rulelist');

        st_rblserver.removeAll();
        st_rulelist.removeAll();

        component.rblserverlist.bindStore(st_rblserver);
        component.spamlist_grid.bindStore(st_rulelist);
    }
