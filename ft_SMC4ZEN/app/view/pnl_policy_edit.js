...
Ext.define('SMC4ZEN.view.pnl_policy_edit', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.pnl_policy_edit',

    requires: [
        'SMC4ZEN.view.pnl_policy_editViewModel',
        'Ext.container.Container',
        'Ext.button.Button'
    ],

    viewModel: {
        type: 'pnl_policy_edit'
    },
    height: 568,
    width: 1253,
    closable: true,
    ...
    onButtonClick4: function(button, e, eOpts) {
        ...
        if(_selrows.length <= 1){
            ...
            if(_selrows[0]){
                _rules['rule'] = Ext.encode(_selrows[0].data);
            }
        ...
        SMC_VIEW.create_grid_panel(
            ...
            function(_grid_define){
                ...
                if(me._kind === OBJECT_SPD_IP_V4_FILTER.TEXT){
                    _search = Ext.create('SMC4ZEN.view.pnl_policy_search_rule_filter',{
                        _policy_cid : me._policy_cid,
                        main_grid : _grid
                    });

                } else if(me._kind === OBJECT_SPD_IP_V4_NAT.TEXT){
                    _search = Ext.create('SMC4ZEN.view.pnl_policy_search_rule_nat',{
                        _policy_cid : me._policy_cid,
                        main_grid : _grid
                    });
                }
    ...
    fn_selected_policy_list: function(_me_obj, _type, _position) {
        ...
        */
        ...
        var createEditWin = function(_type, _win_title, _rule_store, _pos, _kind, _raw){
            ...
            else if(_type === 'update' || _kind === 'obj_spd_ipv4_nat'){
                _width = 900;
                _height = 818;
            }
        ...
        switch (_type){
            ...
            case 'update' :
                ...
                if(_selrows.length > 0){
                    ...
                    _tRaw['@cid'] = _selrows[0].data['@cid'];
                    _tRaw['@groupcid'] = _selrows[0].data['@groupcid'];

                    if(_spd_kind === 'obj_spd_ipv4_filter')
                    {
                        _tRaw['userlev'] = _selrows[0].data['userlev'];
                    }
            ...
            case 'remove' :
                _win_title = '정책삭제';
                if(_selrows.length > 0){
                    createEditWin(_type, _win_title, _rule_list_store, _pos, _spd_kind, _selrows[0].data).show();
                } else {
                    Ext.Msg.show({
                        title : 'Error Message',
                        msg : 'selected policy!',
