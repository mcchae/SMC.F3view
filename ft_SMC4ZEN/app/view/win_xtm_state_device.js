...
Ext.define('SMC4ZEN.view.win_xtm_state_device', {
    extend: 'Ext.window.Window',

    requires: [
        'SMC4ZEN.view.win_xtm_state_deviceViewModel',
        'Ext.tab.Panel',
        'Ext.tab.Tab',
        'Ext.form.FieldSet',
    ...
    listeners: {
        render: 'onWin_xtm_state_deviceRender'
    },
    ...
    onWin_xtm_state_deviceRender: function(component, eOpts) {
        ...
        me.on('deviceStateInfo', this.setDeviceState);
        ...
        try{
            ...
            Ext.Msg.show({
                ...
                msg : '장비 정보를 불러올 수 없습니다. 장비의 연결상태룰 확인하세요.',
                buttons : Ext.Msg.OK,
                icon : Ext.Msg.Error
    ...
    componentStorage: function() {
        ...
        var fds_packet = tpn_main.down('[itemId=pnl_state_status]').down('[itemId=fds_basic_packetstatus]');
        ...
        obj.packetgrid = fds_packet.down('[itemId=gpn_basic_packet]');
    ...
    initStore: function() {
        ...
        // 1. 스토어 객체가 생성되지 않았다면 생성

        if(!hddStore)		hddStore = Ext.create('SMC4ZEN.store.st_state_hdd');
        if(!cpuStore)		cpuStore = Ext.create('SMC4ZEN.store.st_state_cpu_chart');
        if(!ethStore)		ethStore = Ext.create('SMC4ZEN.store.st_state_interface');
        if(!packetStore)	packetStore = Ext.create('SMC4ZEN.store.st_state_packet');
        if(!memoryStore)	memoryStore = Ext.create('SMC4ZEN.store.st_state_memory');
        if(!sessionStore)	sessionStore = Ext.create('SMC4ZEN.store.st_state_session');
        ...
        component.ethgrid.bindStore(ethStore);
        component.hddstatus.bindStore(hddStore);
        component.packetgrid.bindStore(packetStore);
    }
