...
Ext.define('SMC4ZEN.view.pnl_object_svc_http', {
    extend: 'Ext.window.Window',
    alias: 'widget.pnl_object_svc_http',

    requires: [
        'SMC4ZEN.view.pnl_object_svc_httpViewModel',
        'Ext.tab.Panel',
        'Ext.tab.Tab',
        'Ext.form.Label',
    ...
    listeners: {
        afterrender: 'onPnl_object_svc_httpAfterRender',
        beforedestroy: 'onPnl_object_svc_httpBeforeDestroy'
    },
    ...
    loadData: function(record) {
        ...
        var st_HarmfulSite = Ext.create('SMC4ZEN.store.st_HarmfulSite');

        me.down('combobox[itemId="cmb_protocol"]').bindStore(Ext.create('SMC4ZEN.store.st_HttpProtocolType'));
        me.down('combobox[itemId="cmb_site_nude"]').bindStore(st_HarmfulSite);
        me.down('combobox[itemId="cmb_site_sex"]').bindStore(st_HarmfulSite);
        me.down('combobox[itemId="cmb_site_violence"]').bindStore(st_HarmfulSite);
        me.down('combobox[itemId="cmb_site_language"]').bindStore(st_HarmfulSite);

        var protocol_store = me.down('combobox[itemId="cmb_protocol"]').getStore();
        var harmful_store = Ext.getStore('st_HarmfulSite');

        me.down('combobox[itemId="cmb_protocol"]').select(Ext.getStore('st_HttpProtocolType').getAt(0));
        me.down('combobox[itemId="cmb_site_nude"]').select(harmful_store.getAt(0));
        me.down('combobox[itemId="cmb_site_sex"]').select(harmful_store.getAt(0));
        me.down('combobox[itemId="cmb_site_violence"]').select(harmful_store.getAt(0));
        me.down('combobox[itemId="cmb_site_language"]').select(harmful_store.getAt(0));
