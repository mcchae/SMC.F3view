...
Ext.define('SMC.view.trpn_objectMenu', {
    ...
    initComponent: function() {
        ...
        Ext.applyIf(me, {
            ...
            viewConfig: {
                ...
                listeners: {
                    drop: {
                        fn: me.onTrv_objectMenuDrop,
                        scope: me
                    },
                    nodedragover: {
                        fn: me.onTrv_objectMenuNodeDragOver,
                        scope: me
                    },
                    afterrender: {
    ...
    onTrv_objectMenuDrop: function(node, data, overModel, dropPosition, eOpts) {
        var me = this;

        var target_group = overModel;
        var select_group = data.records[0];

        if(target_group.raw.cid && select_group.raw.cid)
        {
            if(target_group.raw.cid !== select_group.raw.cid)
            {
                var _svc = 'ftSMC',
                    _func = 'movGroup',
                    _params = {
                        t_cid : Ext.encode(target_group.raw.cid),
                        cid : Ext.encode(select_group.raw.cid)
                    };
                ...
                request_helper.xmlrpc_call_Ajax_Post(
                    ...
                    function(response){
                        ...
                        if(response)
                        {
                            me.set_obj_tree(select_group.raw);
                        }
                    }
                );
            }
    ...
    onTrv_objectMenuNodeDragOver: function(targetNode, position, dragData, e, eOpts) {
        if(targetNode.raw._kind !== dragData.records[0].raw._kind)
        {
            this.getView().plugins[0].dropZone.lock();
        }
        else if(targetNode.raw._kind === dragData.records[0].raw._kind)
        {
            this.getView().plugins[0].dropZone.unlock();
        }
    },
