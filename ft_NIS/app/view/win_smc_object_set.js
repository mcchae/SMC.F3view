...
Ext.define('SMC.view.win_smc_object_set', {
    extend: 'Ext.window.Window',
    alias: 'widget.smc_object_set',
            ...
            items: [
                {
                    ...
                    searchChildGroup: function(cid, childNodes, record) {
                        ...
                        for(var i = 0, max = childNodes.length; i < max; i++){
                            ...
                            if(cid === childNodes[i].raw.cid){
                                ...
                                var selectNode = trpn_group.getSelectionModel().getSelection()[0];
                    ...
                    listeners: {
                        select: {
                            fn: me.onTrpn_object_groupSelect,
                            scope: me
                        }
                    },
                    dockedItems: [
                        {
                            ...
                            listeners: {
                                afterrender: {
                                    fn: me.onTb_object_controlAfterRender,
                                    scope: me
                                }
                            }
                        }
                ...
                },
                {
                    ...
                    listeners: {
                        itemdblclick: {
                            fn: me.onGpn_object_selectobjItemDblClick,
                            scope: me
                        }
                    }
                },
    ...
    */
    ...
    onGpn_object_selectobjItemDblClick: function(dataview, record, item, index, e, eOpts) {
        ...
        this.thisObj.fireEvent('setPolicy', this.componentObj, this.policyKey, selectRecord.raw);
    ...
    onWindowAfterRender: function(component, eOpts) {
        ...
        this.loadObjectGroup(component);
    },
    ...
    makeContextMenu: function(contextType, insertComponent) {
        ...
        // 일시 : 2014.11.06
        //
        // 설명 : 객체, 정책에 맞는 컨텍스트 메뉴를 생성하여 항목에 추가합니다.
        //
        // ==============================================================================================================================================================================
    ...
    refreshObjectList: function(record, objectStore) {
        ...
        var me = this;
        var trpn_group = this.down('[itemId=trpn_object_group]');
        var gpn_objectlist = this.down('[itemId=gpn_object_selectobj]');
                        ...
                        objectStore.loadPage(pageNumber, {
                            ...
                            'callback' : function(records){
                                ...
                                for(var i = 0, max = objectStore.count(); i < max; i++){
                                    ...
                                    if(objectStore.data.items[i].raw['@cid'] === me.componentObj.policyCid){
    ...
    makeObjectWnd: function(kind) {
        ...
        switch(kind){
            ...
            case "obj_ip_v4_addr":
                wndInstance = Ext.create('SMC.view.pnl_object_ipv4_addr');
                break;
            case "obj_ip_v6_addr":
                wndInstance = Ext.create('SMC.view.pnl_object_ipv6_addr');
                break;
            case "obj_ip_v4_group":
                wndInstance = Ext.create('SMC.view.pnl_object_ipv4_addr_group');
                break;
            case "obj_ip_v6_group":
                wndInstance = Ext.create('SMC.view.pnl_object_ipv6_addr_group');
                break;
            case "obj_ip_v6_header":
                wndInstance = Ext.create('SMC.view.pnl_object_ipv6_header');
                break;
            case "obj_svc_port":
                wndInstance = Ext.create('SMC.view.pnl_object_svc_port');
                break;
            case "obj_svc_group":
                wndInstance = Ext.create('SMC.view.pnl_object_svc_group');
                break;
            case "obj_svc_http":
                wndInstance = Ext.create('SMC.view.pnl_object_svc_http');
                break;
            case "obj_svc_rpc":
                wndInstance = Ext.create('SMC.view.pnl_object_svc_rpc');
                break;
            case "obj_svc_url":
                wndInstance = Ext.create('SMC.view.pnl_object_svc_url');
                break;
            case "obj_schedule":
                wndInstance = Ext.create('SMC.view.pnl_object_schedule');
                break;
            case "obj_ipsec_isakmpsa":
                wndInstance = Ext.create('SMC.view.pnl_object_isakmp_sa');
                break;
            case "obj_ipsec_ipsecsa":
                wndInstance = Ext.create('SMC.view.pnl_object_ipsec_sa');
                break;
            case "obj_ipsec_host":
                wndInstance = Ext.create('SMC.view.pnl_object_sec_host');
                break;
            case "obj_ipsec_peer":
                wndInstance = Ext.create('SMC.view.pnl_object_ipsec_gate');
                break;
            case "obj_qos":
                wndInstance = Ext.create('SMC.view.pnl_object_qos');
                break;
            case "obj_session_limit":
                wndInstance = Ext.create('SMC.view.pnl_object_session_limit');
                break;
            case "obj_usr_list":
                wndInstance = Ext.create('SMC.view.pnl_object_user');
                break;
            case "obj_ssl_svr":
                wndInstance = Ext.create('SMC.view.pnl_object_ssl_server');
                break;
            case "obj_ssl_svr_group":
                wndInstance = Ext.create('SMC.view.pnl_object_ssl_server_group');
                break;
            case "obj_ssl_usr_list":
                wndInstance = Ext.create('SMC.view.pnl_object_ssl_user_group');
                record.raw._kind = 'obj_ssl_usr';
                break;
            case "obj_usr_group":
                wndInstance = Ext.create('SMC.view.pnl_object_user_group');
                break;
            case "obj_waf":
                wndInstance = Ext.create('SMC.view.pnl_object_waf');
                break;
            default:
                wndInstance = null;
                break;
            ...
            function(res){
                ...
                if(res){
                    ...
                    tree.getSelectionModel().store.each(function(data, idx){
                        ...
                        if(data.raw.gtype === res._kind){

                            if(data.raw.cid === res['@groupcid']){

                                var tree_selection = tree.getSelectionModel().getSelection()[0];
            ...
            objectContextData = [
                        ...
                        request_helper.xmlrpc_call_Ajax_Post(
                            ...
                            function(response){
                                ...
                                if(wndInstance){
                                    ...
                                    response['@groupcid'] = component.objtree.getSelectionModel().getSelection()[0].raw.cid;
                    ...
                    'callback' : function(){
                        ...
                        var _svc = 'ftSMC',
                            ...
                            _params = {
                                ...
                                'cid' : Ext.encode(selectRecord.raw['@cid'])
                ...
                },
                {
                    ...
                    'callback' : function(){
                        ...
                        devFuncModule.showLinkObject(selectRecord.raw['@cid'], selectRecord.raw, 'widget.pnl_object_hierarchy');
